...
export function getCushionList (shopifyCushions) {
    ...
    shopifyCushions.forEach(product => {
        // First Check if the new images are available
        if(product?.metafields?.cushion_large !== undefined && product?.metafields?.cushion_thumbnail !== undefined) {
            cushionList.push({
                thumbnail: product.metafields.cushion_thumbnail_src,
                file: product.metafields.cushion_large_src,
                ...product
            });
            return;
        }
        // If not, check if the old images (Spoonflower) are available
